...
// Add guest
router.post("/", authMiddleware, upload.array("aadharImages", 5), async (req, res) => {
  try {
    const aadharImageUrls = req.files?.map(file => file.secure_url || file.path) || [];
...
router.get("/filter", authMiddleware, async (req, res) => {
  ...
  if (date) {
    ...
    nextDay.setDate(target.getDate() + 1);
    query.arrivalDate = {
      $gte: target.toISOString().split("T")[0],
...
// Update guest
router.put("/:id", authMiddleware, upload.array("aadharImages", 5), async (req, res) => {
  try {
    const aadharImageUrls = req.files?.map(file => file.secure_url || file.path) || [];
    ...
    if (aadharImageUrls.length > 0) {
      updateData.aadharImages = aadharImageUrls;
    }
